...
{
    printf("%s\n", BANNER);
    printf("Welcome to ArtInChip Luban-Lite %d.%d [uC/OS-II - Built on %s %s]\n",
...
static void console_loop_thread(void *arg)
{
    ...
    /* 'console_loop_thread' is Created by 'aicos_thread_create()',
     *then must be deleted by 'aicos_thread_delete'
     */
...
#ifdef AIC_CONSOLE_BARE_DRV
    tshell = aicos_thread_create("shell", 4096, 20, console_loop_thread, NULL);
    if (tshell == NULL) {
...
int main(void)
{
    ...
    /* 'main' thread is created by OSTaskCreateExt,
     * So it must be deteled by OSTaskDel
     */
