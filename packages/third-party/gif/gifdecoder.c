...
static gd_gif * gif_open(gd_gif * gif_base)
{
    ...
    stride = (width * 4 + 7)&(~7);
    gif = GIF_IMG_ALLOC(sizeof(gd_gif) + 4 * stride * height + width * height);
...
#if GIF_COLOR_FORMAT == 0
    gif->frame = &gif->canvas[4 * stride * height];
#elif GIF_COLOR_FORMAT == 14
    gif->frame = &gif->canvas[2 * stride * height];
#endif
    if (gif->bgindex) {
...
#elif GIF_COLOR_FORMAT == 14
        gif_color_t c = gif_color_make(*(bgcolor + 0), *(bgcolor + 1), *(bgcolor + 2));
        gif->canvas[i*2 + 0] = c.full & 0xff;
        gif->canvas[i*2 + 1] = (c.full >> 8) & 0xff;
#endif
    }
...
static Table * new_table(int key_size)
{
    ...
    Table *table = GIF_MEM_ALLOC(sizeof(*table) + sizeof(Entry) * init_bulk);
    if (table) {
...
static int add_entry(Table **tablep, uint16_t length, uint16_t prefix, uint8_t suffix)
{
    ...
    if (table->nentries == table->bulk) {
        table->bulk *= 2;
        table = GIF_MEM_REALLOC(table, sizeof(*table) + sizeof(Entry) * table->bulk);
        if (!table) return -1;
        table->entries = (Entry *) &table[1];
        *tablep = table;
    }
...
static void render_frame_rect(gd_gif *gif, uint8_t *buffer)
{
    ...
    int width = 0;
#if GIF_COLOR_FORMAT == 0
    width = ((gif->width*4+7)&(~7))/4;
#elif GIF_COLOR_FORMAT == 14
    width = ((gif->width*2+7)&(~7))/2;
#endif
    //i = gif->fy * gif->width + gif->fx;
    i = gif->fy * width + gif->fx;
    for (j = 0; j < gif->fh; j++) {
        for (k = 0; k < gif->fw; k++) {
            ...
            if (!gif->gce.transparency || index != gif->gce.tindex) {
                ...
                buffer[(i+k)*4 + 3] = 0xFF;
#elif GIF_COLOR_FORMAT == 14
                gif_color_t c = gif_color_make(*(color + 0), *(color + 1), *(color + 2));
                buffer[(i+k)*2 + 0] = c.full & 0xff;
                buffer[(i+k)*2 + 1] = (c.full >> 8) & 0xff;
#endif
            }
        }
        //i += gif->width;
    ...
    case 2: /* Restore to background color. */
        ...
        for (j = 0; j < gif->fh; j++) {
            for (k = 0; k < gif->fw; k++) {
                ...
                gif_color_t c = gif_color_make(*(bgcolor + 0), *(bgcolor + 1), *(bgcolor + 2));
                gif->canvas[(i+k)*2 + 0] = c.full & 0xff;
                gif->canvas[(i+k)*2 + 1] = (c.full >> 8) & 0xff;
#endif
            }
            //i += gif->width;
            i += width;
...
static bool f_gif_open(gd_gif * gif, const void * path, bool is_file)
{
    ...
    if(is_file) {
        gif->fd = open(path,O_RDONLY);
        if(gif->fd < 0) return false;
        else return true;
    } else {
        gif->data = path;
        return true;
