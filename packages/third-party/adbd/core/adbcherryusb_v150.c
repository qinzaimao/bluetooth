...
void usbd_winusb_out(uint8_t busid, uint8_t ep, uint32_t size);
void usbd_winusb_in(uint8_t busid, uint8_t ep, uint32_t size);
...
static rt_err_t usbd_interface_enable(winusb_device_t wd)
{
    ...
    usbd_ep_start_read(0, winusb_out_ep1.ep_addr, read_buffer, WINUSB_EP_MPS);
...
void usbd_comp_adb_event_handler(uint8_t busid, uint8_t event)
{
...
static int _file_write(struct dfs_fd *fd, const void *buf, size_t size)
{
    ...
    usbd_ep_start_write(0, winusb_in_ep1.ep_addr, buf, wlen);
...
{
    usbd_add_interface(0, usbd_adb_init_intf(&adb_intf0, ep_table[0], ep_table[1]));
    return 0;
}
#endif
...
int adb_winusb_init(void)
{
    ...
    usbd_desc_register(0, winusb_descriptor);
    usbd_add_interface(0, usbd_adb_init_intf(&adb_intf0, WINUSB_OUT_EP, WINUSB_IN_EP));
    usbd_initialize(0, USB_DEV_BASE, usbd_comp_adb_event_handler);
#else
