...
static void copyfile(const char *src, const char *dst)
{
    ...
    block_ptr = (rt_uint8_t *)rt_malloc_align(BUF_SZ, CACHE_LINE_SIZE);
    if (block_ptr == NULL)
    {
...
#define FLAG_DST_NON_EXSIT 0x00
    ...
    if (flag & FLAG_SRC_IS_FILE)
    {
        ...
        else
        {
            ...
            speed = (float)filesize / 1024 / 1024 / time_diff(&start, &end);
            printf("Copy data %d Byte, speed %d.%02d MB/s\n", (int)filesize,
