...
#define DPT_ITEM_SIZE   16          /* partition item size */
    ...
    rt_kprintf("found part[%d], begin: %d, size: ",
               (u32)pindex, (u32)part->offset * 512);
    if ((part->size >> 11) == 0)
        rt_kprintf("%d%s", (u32)part->size >> 1, "KB\n"); /* KB */
    else
    {
        unsigned int part_size;
        part_size = part->size >> 11;                /* MB */
        if ((part_size >> 10) == 0)
            rt_kprintf("%d.%d%s", part_size, (u32)(part->size >> 1) & 0x3FF, "MB\n");
        else
...
int dfs_mount_table(unsigned int prio)
{
    int index = 0;

    while (1)
    {
        if (mount_table[index].path == NULL)
            break;
        if (mount_table[index].prio != prio) {
            index ++;
            continue;
        }
...
int dfs_mount_table_prio0(void)
{
    return dfs_mount_table(0);
}

int dfs_mount_table_prio1(void)
{
    return dfs_mount_table(1);
}
...
int dfs_mount_device(rt_device_t dev)
{
  ...
  while (1)
  {
    ...
    /* which is a non-device filesystem mount */
    if (mount_table[index].device_name == NULL)
    {
        /* non-device filesystem can't use dfs_mount_device */
        index ++;
        continue;
    }
