...
extern void gettimespec(struct timespec *t);
...
static void run_once(long count)
{
    long long tsc_begin;
    ...
    freq = count * 200.0 / (us_end250 - us_end50);
    if (use_ints)
        snprintf(mhz, sizeof(mhz), "%d", (int)(freq + 0.5));
    else
        snprintf(mhz, sizeof(mhz), "%d.%02d", (int)freq, (int)(freq * 100) % 100);

    if (!tsc_only) {
        if (!cpu_only) {
            printf("count: %ld, us50: %lld, us250: %lld, diff: %lld\n",
 ...
 */
static void pre_heat(long delay)
{
...
{
    printf("Usage: %s [-h|-v%s]* [lines [heat [count]]]\n"
           "  -h      show this help\n"
...
static int cmd_mhz(int argc, char **argv)
{
...
#ifdef RT_USING_FINSH
MSH_CMD_EXPORT_ALIAS(cmd_mhz, mhz, "calculate CPU frequency");
#endif
