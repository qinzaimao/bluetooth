...
int main2(int argc, char *argv[])
#else
int minimp3_test(int argc, char *argv[])
#endif
{
    int i, ref_size, do_self_test = 0;

    if (argc < 5) {
        printf("Usage:%s -m 6 /sdcard/test.mp3 /sdcard/test.wav\n", __func__);
        return -1;
    }
    ...
    char *ref_file_name    = (argc >= (i + 1)) ? argv[i] : NULL;
    char *output_file_name = (argc >= (i + 2)) ? argv[i + 1] : NULL;
    FILE *file_out = NULL;
    if (output_file_name)
    {
...
int minimp3_stream_demo(int argc, char *argv[])
{
    int buf_size = 4096, data_bytes = 0;
    int bytes_read = 0,  bytes_left = 0,  dec_len = 0;
    int remain_sample_bytes = 0, samples_bytes = 0;
    int once_write_bytes = 1024, write_cnt = 0;;
    ...
    mp3_data = malloc(buf_size * 2);
    pcm_data = malloc(MINIMP3_MAX_SAMPLES_PER_FRAME * sizeof(short) * 2);
    ...
    do {
        dec_len = 0;
        bytes_read = bytes_left + fread(mp3_data + bytes_left, 1, buf_size, file_in);
#ifdef MINIMP3_DEMO_DEBUG
        printf("read size:%d, bytes_left:%d, dec_len:%d, remain_sample_bytes=%d\n",
            bytes_read, bytes_left, dec_len, remain_sample_bytes);
#endif
        do {
            samples_bytes = sizeof(short) * mp3dec_decode_frame(&mp3d, mp3_data + dec_len,
                bytes_read - dec_len, (short *)(pcm_data + remain_sample_bytes), &info);

            remain_sample_bytes += samples_bytes * info.channels;
            dec_len += info.frame_bytes;
            write_cnt = 0;
#ifdef MINIMP3_DEMO_DEBUG
            printf("remain_sample_bytes=%d, samples_bytes=%d, bytes_read=%d, dec_len=%d,"
                   "info: %dhz, channels=%d, frame_bytes=%d, frame_offset=%d\n",
                   remain_sample_bytes, samples_bytes, bytes_read, dec_len, info.hz, info.channels,
                   info.frame_bytes, info.frame_offset);
#endif
            while (remain_sample_bytes >= once_write_bytes) {
                fwrite(pcm_data + write_cnt * once_write_bytes,
                       1, once_write_bytes, file_out);
                write_cnt++;
                remain_sample_bytes -= once_write_bytes;
            }
#ifdef MINIMP3_DEMO_DEBUG
            if (write_cnt)
                printf("write pcm size: %d, remain_sample_bytes:%d\n",
                    write_cnt * once_write_bytes, remain_sample_bytes);
#endif
            if (remain_sample_bytes != 0) {
                memcpy(pcm_data, pcm_data + write_cnt * once_write_bytes,
                       remain_sample_bytes);
#ifdef MINIMP3_DEMO_DEBUG
                printf("memcpy remainsize=%d, samples_bytes=%d\n",
                        remain_sample_bytes, samples_bytes);
#endif
            }

            if (samples_bytes == 0) {
                fwrite(pcm_data, 1, remain_sample_bytes, file_out);
#ifdef MINIMP3_DEMO_DEBUG
                printf("write remain pcm size: %d\n", remain_sample_bytes);
#endif
                remain_sample_bytes = 0;
                break;
            }

            if (bytes_read - dec_len < info.frame_bytes * 2) {
                bytes_left = bytes_read - dec_len;
                memcpy(mp3_data, mp3_data + dec_len, bytes_left);
#ifdef MINIMP3_DEMO_DEBUG
                printf("memcpy data to mp3data size: %d\n", bytes_left);
#endif
                break;
            } else {
                bytes_left = 0;
#ifdef MINIMP3_DEMO_DEBUG
                printf("bytes_left size: %d\n", bytes_left);
#endif
            }
        } while (samples_bytes > 0);
    } while (bytes_read > 0);

    data_bytes = ftell(file_out) - 44;
    rewind(file_out);
    fwrite(wav_header(info.hz, info.channels, 16, data_bytes), 1, 44, file_out);
    printf("write headerinfo :%dhz, channels=%d\n", info.hz, info.channels);
    free(mp3_data);
    free(pcm_data);
    fclose(file_in);
    fclose(file_out);
...
MSH_CMD_EXPORT(minimp3_test, minimp3 internal decoder test);
MSH_CMD_EXPORT(minimp3_stream_demo, minimp3 self decoder test);
