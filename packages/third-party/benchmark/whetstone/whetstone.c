...
{
    return (long)(aic_get_time_us()/1000000);
}
...
int
whetstone_main(int argc, char *argv[])
{
	...
	loopstart = WHETSTONE_ITERATIONS;
#else
...
*/
	...
	if (KIPS >= 1000.0) {
		...
		p_i1 = (unsigned int)p_f;
		p_i2 = (unsigned int)((p_f-p_i1)*100.0);
		printf("Score(CPU %ldMHz): %d.%d\n", CPU_FREQ, p_i1, p_i2);
	}
	else {
		p_f = KIPS;
		p_i1 = (unsigned int)p_f;
...
void test_whetstone()
{
    char *args[] = {"app", "1000000"};

    whetstone_main(0, args);
}

MSH_CMD_EXPORT(test_whetstone, "Whetstone benchmark")
