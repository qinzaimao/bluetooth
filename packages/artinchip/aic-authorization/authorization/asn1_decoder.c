...
/* Corresponds to CONS bit in the opcode tag
...
next_op:
    ...
    switch (op) {
        ...
        case ASN1_OP_COND_MATCH_ANY_ACT_OR_SKIP:
            ...
            if (!(flags & FLAG_CONS)) {
                if (flags & FLAG_INDEFINITE_LENGTH) {
                    ...
                    ret = asn1_find_indefinite_length(data, datalen, &tmp, &len,
                                                      &errmsg);
                    if (ret < 0)
                        goto error;
                }
            ...
            if (op & ASN1_OP_MATCH__ACT) {
                ...
                if (op & ASN1_OP_MATCH__ANY)
                    ...
                    act = machine[pc + 2];
                ret = actions[act](context, data + dp, len);
                if (ret < 0)
                    return ret;
            }
        ...
        case ASN1_OP_END_SEQ_OF_ACT:
            ...
            if (op & ASN1_OP_END__ACT) {
                ...
                if (op & ASN1_OP_END__OF)
                    ...
                    act = machine[pc + 1];
                ret = actions[act](context, data + tdp, len);
                if (ret < 0)
                    return ret;
            }
            pc += asn1_op_lengths[op];
        ...
        case ASN1_OP_ACT:
            ret = actions[machine[pc + 1]](context, data + tdp, len);
            if (ret < 0)
                return ret;
            pc += asn1_op_lengths[op];
            goto next_op;
