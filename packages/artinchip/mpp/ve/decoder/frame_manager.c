...
struct frame_manager *fm_create(struct frame_manager_init_cfg *init_cfg)
{
    ...
    logi("frame info: stride: %d, height_align: %d, cnt: %d",
         init_cfg->stride, init_cfg->height_align, init_cfg->frame_count);
    ...
    frm_manager->frame_node =
        (struct frame_impl *)mpp_alloc(frm_manager->frame_count * sizeof(struct frame_impl));
    ...
    for (i = 0; i < frm_manager->frame_count; i++) {
        ...
        ret = frm_manager->allocator->ops->alloc_frame_buffer(frm_manager->allocator,
                                                              &frm_impl->frm.mpp_frame,
                                                              init_cfg->stride,
                                                              init_cfg->height_align,
                                                              init_cfg->pixel_format);
        if (ret < 0)
            break;
...
int fm_decoder_put_frame(struct frame_manager *fm, struct frame *frame)
{
    ...
    logd("fm_decoder_put_frame, ref_count: %d, displayed: %d",
         frm_impl->ref_count, frm_impl->displayed);
