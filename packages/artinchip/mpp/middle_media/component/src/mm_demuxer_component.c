...
static int demuxer_thread_attr_init(pthread_attr_t *attr)
{
    ...
    attr->schedparam.sched_priority = MM_COMPONENT_DEMUXER_THREAD_PRIORITY;
    return 0;
}
...
s32 mm_demuxer_component_init(mm_handle h_component)
{
    ...
    err = pthread_create(&p_demuxer_data->thread_id, &attr,
                         mm_demuxer_component_thread, p_demuxer_data);
    //if (err || !p_demuxer_data->thread_id)
    if (err) {
                                       ...
                                       struct aic_parser_packet *p_pkt)
{
    ...
    if (p_demuxer_data->extra_video_pkt_flag) {
        ...
        logi("send video extra data,pts:%lld,type = %d,size:%d,flag:0x%x\n",
    ...
    if (ret != 0) {
        ...
        mm_send_command(h_vdec_comp, MM_COMMAND_NOPS, 0, NULL);
        p_demuxer_data->put_video_pkt_ok_num++;
    }
                                       ...
                                       struct aic_parser_packet *p_pkt)
{
    ...
    if (p_demuxer_data->extra_audio_pkt_flag) {
        ...
        logi("send audio extra data,pts:%lld,type = %d,size:%d,flag:0x%x\n",
             p_pkt->pts, p_pkt->type, p_pkt->size, p_pkt->flag);
    ...
    if (ret != 0) {
        ...
        mm_send_command(h_adec_comp, MM_COMMAND_NOPS, 0, NULL);
        p_demuxer_data->put_audio_pkt_ok_num++;
    }
    return ret;
}
