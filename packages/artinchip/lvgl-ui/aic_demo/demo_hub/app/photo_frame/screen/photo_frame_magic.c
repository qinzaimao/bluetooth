...
lv_obj_t *photo_frame_magic;

lv_obj_t *photo_frame_magic_create(lv_obj_t *parent)
{
    photo_frame_magic = lv_img_create(parent);
    lv_img_set_src(photo_frame_magic, LVGL_PATH(photo_frame/bg.jpg));
    lv_obj_clear_flag(photo_frame_magic, LV_OBJ_FLAG_SCROLLABLE);

    lv_obj_t *title = lv_img_create(photo_frame_magic);
    lv_img_set_src(title, LVGL_PATH(photo_frame/magic_page_title.png));
    lv_obj_set_pos(title, adaptive_width(0), adaptive_height(50));
    lv_obj_set_align(title, LV_ALIGN_TOP_MID);

    lv_obj_t *tip = lv_img_create(photo_frame_magic);
    lv_img_set_src(tip, LVGL_PATH(photo_frame/video_page_tip.png));
    lv_obj_set_align(tip, LV_ALIGN_CENTER);

    lv_obj_t *home_btn = com_imgbtn_comp(photo_frame_magic, LVGL_PATH(photo_frame/home.png), NULL);
...
static void back_main_cb(lv_event_t * e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        lv_obj_add_flag(photo_frame_magic, LV_OBJ_FLAG_HIDDEN);
    }
}
