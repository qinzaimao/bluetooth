...
static rt_mutex_t lv_cache_mutex = RT_NULL;
#endif
...
static void lv_mutex_init(void);
static void lv_mutex_lock(void);
static void lv_mutex_unlock(void);
...
/* Release the cache with the smallest lifecycle */
bool lv_drop_one_cached_image(void)
{
#if LV_IMG_CACHE_DEF_SIZE
    if(entry_cnt == 0) {
        LV_LOG_INFO("lv_drop_one_cached_image: the cache size is 0");
        return false;
    }
    ...
    int32_t i;
    for(i = 0; i < entry_cnt; i++) {
        if(cache[i].dec_dsc.src && !cache[i].used) {
            if(!cached_src)
                cached_src = &cache[i];
...
{
#if defined(KERNEL_RTTHREAD)
    lv_cache_mutex = rt_mutex_create("lv_cache_mutex", RT_IPC_FLAG_PRIO);
    if (lv_cache_mutex == RT_NULL)
    {
        LV_LOG_WARN("lv_drop_one_cached_image: create dynamic mutex failed.\n");
    }
...
#if defined(KERNEL_RTTHREAD)
    if (lv_cache_mutex) {
        rt_mutex_take(lv_cache_mutex, RT_WAITING_FOREVER);
    }
#endif
}
...
#if defined(KERNEL_RTTHREAD)
    if (lv_cache_mutex) {
        rt_mutex_release(lv_cache_mutex);
    }
#endif
}
