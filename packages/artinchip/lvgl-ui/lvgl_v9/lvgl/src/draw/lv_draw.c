...
bool static _lv_area_intersect_aligned(lv_area_t * res_p, const lv_area_t * a1_p, const lv_area_t * a2_p)
{
    /*Get the smaller area from 'a1_p' and 'a2_p'*/
    res_p->x1 = LV_MAX(a1_p->x1 - CACHE_LINE_SIZE / 2, a2_p->x1 - CACHE_LINE_SIZE / 2);
    res_p->y1 = LV_MAX(a1_p->y1, a2_p->y1);
    res_p->x2 = LV_MIN(a1_p->x2 + CACHE_LINE_SIZE / 2, a2_p->x2 + CACHE_LINE_SIZE / 2);
...
static bool is_independent(lv_layer_t * layer, lv_draw_task_t * t_check)
{
    ...
    while(t && t != t_check) {
        if(t->state != LV_DRAW_TASK_STATE_READY) {
            lv_area_t a;
            if(_lv_area_intersect_aligned(&a, &t->_real_area, &t_check->_real_area)) {
                LV_PROFILER_END;
                return false;
            }
        }
