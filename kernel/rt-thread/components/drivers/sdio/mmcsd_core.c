...
#ifdef RT_MMCSD_DBG
static void mmcsd_trace_before_send(struct rt_mmcsd_cmd *cmd)
{
    mmcsd_dbg("CMD_SEND: %d\n", cmd->cmd_code);
    mmcsd_dbg("\t\tARG\t\t\t 0x%08x\n", cmd->arg);
}

static void mmcsd_trace_after_send(struct rt_mmcsd_cmd *cmd)
{
    int i;
    rt_uint8_t *ptr;
    ...
    case RESP_R6:
    case RESP_R7:
        mmcsd_dbg("\t\tRSP_R1,5,6,7 \t\t 0x%08x \n",
            cmd->resp[0]);
        break;
    case RESP_R1B:
        mmcsd_dbg("\t\tRSP_R1B\t\t 0x%08x \n",
            cmd->resp[0]);
        break;
    case RESP_R2:
        mmcsd_dbg("\t\tRSP_R2\t\t\t 0x%08x \n",
            cmd->resp[0]);
        mmcsd_dbg("\t\t          \t\t 0x%08x \n",
            cmd->resp[1]);
        mmcsd_dbg("\t\t          \t\t 0x%08x \n",
        ...
        mmcsd_dbg("\t\t          \t\t 0x%08x \n",
            cmd->resp[3]);
        mmcsd_dbg("\n");
        mmcsd_dbg("\t\t\t\t\tDUMPING DATA\n");
        for (i = 0; i < 4; i++) {
            int j;
            mmcsd_dbg("\t\t\t\t\t%03d - ", i*4);
            ptr = (rt_uint8_t *)&cmd->resp[i];
            ptr += 3;
            for (j = 0; j < 4; j++)
                mmcsd_dbg("%02x ", *ptr--);
            mmcsd_dbg("\n");
    ...
    case RESP_R3:
    case RESP_R4:
        mmcsd_dbg("\t\tRSP_R3,4\t\t 0x%08x \n",
            cmd->resp[0]);
        break;
    default:
        mmcsd_dbg("\t\tERROR MMCSD rsp not supported %d\n", resp_type(cmd));
        break;
    }
}
...
void mmcsd_send_request(struct rt_mmcsd_host *host, struct rt_mmcsd_req *req)
{
    do {
        ...
        mmcsd_trace_before_send(req->cmd);
        host->ops->request(host, req);
        mmcsd_trace_after_send(req->cmd);

        rt_sem_take(&host->sem_ack, RT_WAITING_FOREVER);

        if ((req->stop) && (!req->cmd->err)) {
            struct rt_mmcsd_req req_stop = {0};
            req_stop.cmd = req->stop;

            mmcsd_trace_before_send(req_stop.cmd);
            host->ops->request(host, &req_stop);
            mmcsd_trace_after_send(req_stop.cmd);
