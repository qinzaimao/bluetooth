...
static void _pm_device_frequency_change(rt_uint8_t mode)
{
    ...
    for (index = 0; index < _pm.device_pm_late_number; index ++)
    {
        if (_pm.device_pm_late[index].ops->frequency_change != RT_NULL)
            _pm.device_pm_late[index].ops->frequency_change(_pm.device_pm_late[index].device, mode);
    }
}
...
void rt_pm_module_release(uint8_t module_id, rt_uint8_t mode)
{
    ...
    if (pm->modes[mode] > 0)
        pm->modes[mode] --;
    if (pm->module_status[module_id].req_status > 0)
...
void rt_pm_device_register(struct rt_device *device, const struct rt_device_pm_ops *ops)
{
    ...
    struct rt_device_pm *device_pm_late;

    RT_DEBUG_NOT_IN_INTERRUPT;

    level = rt_hw_interrupt_disable();

    if (ops->suspend_late || ops->resume_early)
    {
        device_pm_late = (struct rt_device_pm *)RT_KERNEL_REALLOC(_pm.device_pm_late,
                (_pm.device_pm_late_number + 1) * sizeof(struct rt_device_pm));
        if (device_pm_late != RT_NULL)
        {
            _pm.device_pm_late = device_pm_late;
            _pm.device_pm_late[_pm.device_pm_late_number].device = device;
            _pm.device_pm_late[_pm.device_pm_late_number].ops    = ops;
            _pm.device_pm_late_number += 1;
        }
    }
    else
    {
...
void rt_pm_device_unregister(struct rt_device *device)
{
    ...
    for (index = 0; index < _pm.device_pm_late_number; index ++)
    {
        if (_pm.device_pm_late[index].device == device)
        {
            /* remove current entry */
            for (; index < _pm.device_pm_late_number - 1; index ++)
            {
                _pm.device_pm_late[index] = _pm.device_pm_late[index + 1];
            }

            _pm.device_pm_late[_pm.device_pm_late_number - 1].device = RT_NULL;
            _pm.device_pm_late[_pm.device_pm_late_number - 1].ops = RT_NULL;
