...
struct rt_device_pm_ops
{
    ...
    int (*suspend_late)(const struct rt_device *device, rt_uint8_t mode);
    void (*resume_early)(const struct rt_device *device, rt_uint8_t mode);
    int (*frequency_change)(const struct rt_device *device, rt_uint8_t mode);
};

#define SET_DEVICE_PM_OPS(suspend_fn, resume_fn)            \
    .suspend = suspend_fn,                                  \
    .resume = resume_fn,                                    \

#define SET_LATE_DEVICE_PM_OPS(suspend_fn, resume_fn)       \
...
struct rt_pm
{
    ...
    /* the list of device, which has PM suspend_late resume_early feature */
    rt_uint8_t device_pm_late_number;
