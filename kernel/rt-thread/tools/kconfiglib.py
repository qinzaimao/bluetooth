            ...
            for low_expr, high_expr, cond in self.ranges:
                if expr_value(cond):
                    ...
                    if _is_base_n(low_expr.str_value, base):
                        ...
                        self.kconfig._warn(
                            "low value {} is not match the base {} for symbol {}"
                            .format(low_expr.str_value, base, self.name_and_loc))
                        if low_expr.str_value.startswith("0x") or low_expr.str_value.startswith("0X"):
                            low = int(low_expr.str_value, 16)
                    high = 0
                    if _is_base_n(high_expr.str_value, base):
                        high = int(high_expr.str_value, base)
                    else:
                        self.kconfig._warn(
                            "high value {} is not match the base {} for symbol {}"
                            .format(high_expr.str_value, base, self.name_and_loc))
                        if high_expr.str_value.startswith("0x") or high_expr.str_value.startswith("0X"):
                            high = int(high_expr.str_value, 16)
                    break
            else:
