...
#define SPINAND_MFR_XINCUN1 0x6C
...
int xcsp1x_ecc_get_status(struct aic_spinand *flash, u8 status)
{
    switch (status & STATUS_ECC_MASK) {
        case STATUS_ECC_NO_BITFLIPS:
            return 0;
...
const struct aic_spinand_info xincun_spinand_table[] = {
    ...
    { DEVID(0x01), PAGESIZE(2048), OOBSIZE(64), BPL(1024), PPB(64),
      ...
      xincun_ecc_get_status, xcsp1a_ooblayout_user, 8 },
};

const struct aic_spinand_info xincun1_spinand_table[] = {
    /*devid page_size oob_size block_per_lun pages_per_eraseblock planes_per_lun
    is_die_select*/
    /*XCSP1xXPK-IT device*/
    { DEVID(0x01), PAGESIZE(2048), OOBSIZE(128), BPL(1024), PPB(64),
      PLANENUM(1), DIE(0), "xincun 128MB: 2048+128@64@1024", cmd_cfg_table,
      xcsp1x_ecc_get_status, xcsp1a_ooblayout_user, 9 },
};
...
xincun_spinand_detect(struct aic_spinand *flash)
{
    ...
    if (id[0] == SPINAND_MFR_XINCUN)
        return spinand_match_and_init(&id[1], xincun_spinand_table, ARRAY_SIZE(xincun_spinand_table));

    if (id[0] == SPINAND_MFR_XINCUN1)
        return spinand_match_and_init(&id[1], xincun1_spinand_table, ARRAY_SIZE(xincun1_spinand_table));
