...
#define F35SQB_STATUS_ECC_MASK             GENMASK(6, 4)
#define F35SQB_STATUS_ECC_NO_BITFLIPS      (0)
#define F35SQB_STATUS_ECC_UNCOR_ERROR             GENMASK(6, 4)
...
static int f35sqb_ecc_get_status(struct aic_spinand *flash, u8 status)
{
    switch (status & F35SQB_STATUS_ECC_MASK) {
        case F35SQB_STATUS_ECC_UNCOR_ERROR:
            return -SPINAND_ERR_ECC;
        case F35SQB_STATUS_ECC_NO_BITFLIPS:
            return 0;
        default:
            return ((status & F35SQB_STATUS_ECC_MASK) >> 4) + 2;
            break;
    }
...
static int f35nd04g_ecc_get_status(struct aic_spinand *flash, u8 status)
{
    switch (status & FORESEE_STATUS_ECC_MASK) {
        case FORESEE_STATUS_ECC_NO_BITFLIPS:
            return 0;
...
const struct aic_spinand_info foresee_spinand_table[] = {
    ...
    { DEVID(0x70), PAGESIZE(2048), OOBSIZE(64), BPL(512), PPB(64), PLANENUM(1),
      ...
      foresee_ecc_get_status, f35sq1g_ooblayout_user, 1 },
    /*F35SQA001G*/
