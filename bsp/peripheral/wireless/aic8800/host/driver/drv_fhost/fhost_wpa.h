...
enum fhost_wpa_event {
    ...
    FHOST_WPA_PROCESS_ERROR,

    FHOST_WPA_DPP_NETWORK_ADDED,

    FHOST_WPA_LAST,
};

enum fhost_wpa_error
{
    /// Invalid password
    FHOST_WPA_ERROR_WRONG_KEY,
    /// Error during DPP protocol
    FHOST_WPA_ERROR_DPP,
    /// Join scan limit reached
    FHOST_WPA_ERROR_JOIN_SCAN_LIMIT_REACHED,
};
...
int fhost_wpa_send_event_with_name(enum fhost_wpa_event event, void *param, int param_len,
                                   char *itf_name);
...
int fhost_ap_del_whitelist(int fhost_vif_idx, struct mac_addr *macaddr);
...
int fhost_ap_wps_pbc(int fhost_vif_idx);
int fhost_ap_wps_pin(int fhost_vif_idx,char *pin);
int fhost_ap_wps_cacel(int fhost_vif_idx);
int fhost_ap_wps_disable(int fhost_vif_idx);
