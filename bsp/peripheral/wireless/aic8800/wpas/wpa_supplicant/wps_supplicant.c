...
#include "../../src/eap_server/eap_methods.h"
...
static int hostapd_config_read_eap_user(struct hostapd_bss_config *conf, const char *buf)
{
	//FILE *f;
...
*/
    ...
    do {
		...
		if (buf[0] == '*') {
            ...
            aic_dbg("%s identity: %s\r\n", __func__, user->identity);
		...
		while (*start) {
            ...
            //aic_dbg("%s %d %d\r\n", __func__, user->methods[num_methods].method, user->methods[num_methods].vendor);
...
int wpas_wps_set_eap_user(struct hostapd_iface *ap_iface, char* pos)
{
#ifdef CONFIG_AP
	if (ap_iface) {
		wpa_printf(MSG_DEBUG, "WPS: Setting eap user in AP mode");
        hostapd_config_read_eap_user((struct hostapd_bss_config *)ap_iface->conf, pos);
	}
#endif /* CONFIG_AP */
