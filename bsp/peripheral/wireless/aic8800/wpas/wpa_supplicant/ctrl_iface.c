...
#include "sme.h"
#include "../src/ap/ctrl_iface_ap.h"
					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
	...
	} else if (os_strcasecmp(cmd, "p2p_dev_name") == 0) {
	    if(wpa_s->conf->device_name) {
			aic_dbg("%s free %s\r\n", __func__, wpa_s->conf->device_name);
			os_free(wpa_s->conf->device_name);
		}
		wpa_s->conf->device_name = os_strdup(value);
		aic_dbg("%s dev_name=%s\n", __func__, wpa_s->conf->device_name);
...
static int wmm_ac_ctrl_addts(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	struct wmm_ac_ts_setup_params params = {
		.tsid = 0xff,
		.direction = (enum wmm_ac_dir)0xff,
	};
	...
	while ((token = str_token(cmd, " ", &context))) {
		...
		if (os_strcasecmp(token, "downlink") == 0) {
			params.direction = (enum wmm_ac_dir)WMM_TSPEC_DIRECTION_DOWNLINK;
		} else if (os_strcasecmp(token, "uplink") == 0) {
			params.direction = (enum wmm_ac_dir)WMM_TSPEC_DIRECTION_UPLINK;
		} else if (os_strcasecmp(token, "bidi") == 0) {
			params.direction = (enum wmm_ac_dir)WMM_TSPEC_DIRECTION_BI_DIRECTIONAL;
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	while ((token = str_token(cmd, " ", &context))) {
		if (sscanf(token, "freq2=%d", &freq2) == 1 ||
			...
			persistent = 1;
		}  else if (sscanf(token, "pass=%s", &pass) == 1) {
            continue;
		} else {
			wpa_printf(MSG_DEBUG,
...
static int wpas_ctrl_vendor_elem_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (frame < 0 || frame >= NUM_VENDOR_ELEM_FRAMES)
		return -1;
	wpa_s = wpas_vendor_elem(wpa_s, (enum wpa_vendor_elem_frame)frame);
...
#define NR_IE_MIN_LEN (ETH_ALEN + 4 + 1 + 1 + 1)
	...
	while (len >= 2 + NR_IE_MIN_LEN) {
		...
		wpa_msg(wpa_s, MSG_INFO, RRM_EVENT_NEIGHBOR_REP_RXED
			...
			MAC2STR(nr), (unsigned)WPA_GET_LE32(nr + ETH_ALEN),
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0 ||
		...
		wpa_dbg(wpa_s, wpas_ctrl_cmd_debug_level(buf),
				"Control interface command '%s'", buf);
	}
	...
	#else
	} else if (os_strncmp(buf, "P2P_GROUP_ADD", strlen(buf))== 0) {
	#endif
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "SCAN") == 0) {
		...
		if (wpas_wps_cancel(wpa_s))
			reply_len = -1;
    } else if (os_strcmp(buf, "EAP_USER ") == 0) {
		if (wpas_wps_set_eap_user(wpa_s->ap_iface, buf + 9))
			reply_len = -1;
    }else if (os_strncmp(buf, "WPS_PIN ", 8) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_wps_pin(wpa_s, buf + 8,
										 reply,
		...
		if (wpas_ap_stop_ap(wpa_s))
			reply_len = -1;
	} else if (os_strncmp(buf, "MACADDR_ACL ", 12) == 0) {
		enum macaddr_acl acl = (enum macaddr_acl)atoi(buf + 12);
		...
		wpa_s->ap_iface->bss[0]->conf->macaddr_acl = acl;
	} else if (os_strncmp(buf, "ACCEPT_ACL ", 11) == 0) {
		if (os_strncmp(buf + 11, "ADD_MAC ", 8) == 0) {
			if (!hostapd_ctrl_iface_acl_add_mac(
				    &wpa_s->ap_iface->bss[0]->conf->accept_mac,
				    &wpa_s->ap_iface->bss[0]->conf->num_accept_mac, buf + 19))
				hostapd_disassoc_accept_mac((struct hostapd_data *)wpa_s->ap_iface);
			else
				reply_len = -1;
			...
			reply_len = hostapd_ctrl_iface_acl_show_mac(
				wpa_s->ap_iface->bss[0]->conf->accept_mac,
				wpa_s->ap_iface->bss[0]->conf->num_accept_mac, reply, reply_size);
		} else if (os_strcmp(buf + 11, "CLEAR") == 0) {
			hostapd_ctrl_iface_acl_clear_list(
				&wpa_s->ap_iface->bss[0]->conf->accept_mac,
				&wpa_s->ap_iface->bss[0]->conf->num_accept_mac);
		}
	} else if (os_strncmp(buf, "DENY_ACL ", 9) == 0) {
		if (os_strncmp(buf + 9, "ADD_MAC ", 8) == 0) {
			if (!hostapd_ctrl_iface_acl_add_mac(
				    &wpa_s->ap_iface->bss[0]->conf->deny_mac,
				    &wpa_s->ap_iface->bss[0]->conf->num_deny_mac, buf + 17))
				hostapd_disassoc_deny_mac(wpa_s->ap_iface->bss[0]);
			...
			reply_len = hostapd_ctrl_iface_acl_show_mac(
				wpa_s->ap_iface->bss[0]->conf->deny_mac,
				wpa_s->ap_iface->bss[0]->conf->num_deny_mac, reply, reply_size);
		} else if (os_strcmp(buf + 9, "CLEAR") == 0) {
			hostapd_ctrl_iface_acl_clear_list(
				&wpa_s->ap_iface->bss[0]->conf->deny_mac,
				&wpa_s->ap_iface->bss[0]->conf->num_deny_mac);
		}
#endif /* CONFIG_AP */
						...
						char *buf, size_t *resp_len)
{
	...
	wpa_hexdump_ascii(os_strcmp(buf, "PING") ? MSG_DEBUG: MSG_EXCESSIVE,
