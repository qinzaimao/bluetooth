...
#ifdef CONFIG_IEEE80211W
	params.mgmt_frame_protection = (enum mfp_options)wpas_get_ssid_pmf(wpa_s, ssid);
			     ...
			     const u8 *buf, size_t len)
{
    ...
    aic_dbg("RX EAPOL from " MACSTR "len %d(%d)\r\n", MAC2STR(src_addr), len, rtos_now(0));
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	wpa_s->prev_scan_wildcard = 1; // scan for specific SSID at first time
