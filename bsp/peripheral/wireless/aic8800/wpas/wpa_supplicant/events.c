					      ...
					      int own_request, int update_only)
{
	...
	if (scan_res == NULL) {
		...
		if (data && data->scan_info.external_scan)
			return -1;
#ifdef CONFIG_RWNX
        extern uint8_t g_user_reconnect_scan_limit;
        static uint8_t reconnect_scan_cnt = 0;
        reconnect_scan_cnt++;
        if (reconnect_scan_cnt > g_user_reconnect_scan_limit) {
            fhost_wpa_err_evt_t err_evt;
            reconnect_scan_cnt = 0;
            err_evt.err_type = FHOST_WPA_ERROR_JOIN_SCAN_LIMIT_REACHED;
            fhost_wpa_send_event_with_name(FHOST_WPA_PROCESS_ERROR,
                                           (void *)&err_evt,
                                           sizeof(err_evt), wpa_s->ifname);
            goto scan_work_done;
        }
#endif /* CONFIG_RWNX */
						 ...
						 int locally_generated)
{
	...
	if (could_be_psk_mismatch(wpa_s, reason_code, locally_generated)) {
        ...
        #ifdef CONFIG_RWNX
        {
            ...
            int disconn_reason;
            if (locally_generated)
                disconn_reason = -reason_code;
            else
                disconn_reason = reason_code;
            wpa_dbg(wpa_s, MSG_INFO, "auth fail, WRONG_EKY");
            err_evt.err_type = FHOST_WPA_ERROR_WRONG_KEY;
            err_evt.wrong_key_evt.reason_code = disconn_reason;
            fhost_wpa_send_event_with_name(FHOST_WPA_PROCESS_ERROR,
                                           (void *)&err_evt,
                                           sizeof(err_evt), wpa_s->ifname);
        }
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_RX_MGMT: {
        ...
        //aic_dbg("wpa mgmt %02x from " MACSTR "len %d(%d)\r\n", stype, MAC2STR(mgmt->sa), data->rx_mgmt.frame_len, rtos_now(0));
