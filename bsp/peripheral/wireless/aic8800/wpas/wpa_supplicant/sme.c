			...
			int external, const u8 *sa)
{
	...
	if (status_code != WLAN_STATUS_SUCCESS) {
        #ifdef CONFIG_RWNX
        {
            ...
            wpa_dbg(wpa_s, MSG_INFO, "SME auth fail, WRONG_KEY");
            err_evt.err_type = FHOST_WPA_ERROR_WRONG_KEY;
            err_evt.wrong_key_evt.reason_code = status_code;
...
#endif /* CONFIG_IEEE80211R */
	params.mode = mode;
	params.mgmt_frame_protection = (enum mfp_options)wpa_s->sme.mfp;
