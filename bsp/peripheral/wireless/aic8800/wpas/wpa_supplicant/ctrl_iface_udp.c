...
#else /* CONFIG_CTRL_IFACE_UDP_IPV6 */
	wpa_printf(MSG_DEBUG, "CTRL_IFACE monitor attached %s:%d",
		   aic_inet_ntoa(from->sin_addr), ntohs(from->sin_port));
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
	return 0;
}
	...
	while (dst) {
		...
		if (from->sin_addr.s_addr == dst->addr.sin_addr.s_addr &&
			...
			wpa_printf(MSG_DEBUG, "CTRL_IFACE monitor detached "
				   "%s:%d", aic_inet_ntoa(from->sin_addr),
				   ntohs(from->sin_port));
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
			if (prev == NULL)
				*head = dst->next;
	...
	while (dst) {
		...
		if (from->sin_addr.s_addr == dst->addr.sin_addr.s_addr &&
			...
			wpa_printf(MSG_DEBUG, "CTRL_IFACE changed monitor "
				   "level %s:%d", aic_inet_ntoa(from->sin_addr),
				   ntohs(from->sin_port));
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
					      ...
					      void *sock_ctx)
{
	...
	if (from.sin_addr.s_addr != htonl((127 << 24) | 1)) {
		...
		wpa_printf(MSG_DEBUG, "CTRL: Drop packet from unexpected "
			   "source %s", aic_inet_ntoa(from.sin_addr));
		return;
	}
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
#endif /* CONFIG_CTRL_IFACE_UDP_REMOTE */
	...
	if (reply) {
		...
		sendto(sock, "FAIL (no mem)\n", 14, 0, (struct sockaddr *) &from,
		       fromlen);
	} else if (reply_len == 2) {
...
#else /* CONFIG_CTRL_IFACE_UDP_IPV6 */
			wpa_printf(MSG_DEBUG, "CTRL_IFACE monitor send %s:%d",
				   aic_inet_ntoa(dst->addr.sin_addr),
