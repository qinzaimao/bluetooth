...
// TODO: wpa_supplicant_global_ctrl_iface_init
				     ...
				     fd_set *fds)
{
	...
	for (i = 0; i < table->count; i++) {
		...
		//AIC_LOG_PRINTF("%s %d %d %d %d\n", __FUNCTION__, table->table[i].sock, wpa_private_sock, drv_link_sock_recv, net_eth_link_sock_recv);
		FD_SET(table->table[i].sock, fds);
	}
	//AIC_LOG_PRINTF("%s %x\n", __FUNCTION__, fds->fd_bits[0]);
}
...
#if !defined CONFIG_NATIVE_WINDOWS && !defined CONFIG_RWNX
...
#endif /* CONFIG_ELOOP_POLL */
#ifdef CONFIG_ELOOP_SELECT
		//AIC_LOG_PRINTF("%s readers\n", __FUNCTION__);
		eloop_sock_table_set_fds(&eloop.readers, rfds);
		//AIC_LOG_PRINTF("%s writers\n", __FUNCTION__);
		eloop_sock_table_set_fds(&eloop.writers, wfds);
		//AIC_LOG_PRINTF("%sexceptions\n", __FUNCTION__);
		eloop_sock_table_set_fds(&eloop.exceptions, efds);
		//AIC_LOG_PRINTF("%s max %d %x %x us%d\n", __FUNCTION__, eloop.max_sock, rfds->fd_bits[0], wfds->fd_bits[0], _tv.tv_usec);
