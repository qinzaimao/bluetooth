...
#endif /* CONFIG_DEBUG_FILE */
		//vprintf(fmt, ap);
		int ret = vsnprintf(p_buf, 256, fmt, ap);
		if(ret < 0)
		{
		    AIC_LOG_PRINTF("wpa_printf error\n");
			return;
		}
		AIC_LOG_PRINTF(" %s \n", p_buf);
#ifdef CONFIG_DEBUG_FILE
...
void wpa_debug_param_get(struct wpa_debug_param *param)
{
    if (param) {
        param->wpa_debug_level = MSG_INFO;
        param->wpa_debug_show_keys = 0;
        param->wpa_debug_timestamp = 0;
    }
}
