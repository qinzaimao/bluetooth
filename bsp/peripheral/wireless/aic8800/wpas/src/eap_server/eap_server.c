...
void eap_log_msg(struct eap_sm *sm, const char *fmt, ...)
{
    ...
    buflen = vsnprintf(buf_tmp, BUF_SIZE, fmt, ap) + 1;
...
SM_STATE(EAP, SELECT_ACTION)
{
	...
	sm->decision = (enum eap_sm_decision)eap_sm_Policy_getDecision(sm);
}
...
static EapType eap_sm_Policy_getNextMethod(struct eap_sm *sm, int *vendor)
{
	...
	if (sm->identity == NULL || sm->currentId == -1) {
		...
		*vendor = sm->user->methods[idx].vendor;
		next = (EapType)sm->user->methods[idx].method;
		sm->user_eap_method_index++;
	} else {
		*vendor = EAP_VENDOR_IETF;
		next = EAP_TYPE_NONE;
				   ...
				   struct eap_config *conf)
{
	...
	sm->eap_fast_prov = (enum eap_fast_prov_tag)conf->eap_fast_prov;
