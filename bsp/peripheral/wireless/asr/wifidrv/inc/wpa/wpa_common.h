...
#define NUM_PMKID_CACHE 5

struct pmkid_caching_entry {
...
struct pmkid_caching {
    ...
    struct pmkid_caching_entry pmkid_caching_entry[NUM_PMKID_CACHE];
};
#endif
...
uint16_t wpa_gen_wpa_ie_rsn(u8 *rsn_ie, size_t rsn_ie_len,
                  int pairwise_cipher, int group_cipher,
                  int key_mgmt, int mgmt_group_cipher,
                   struct pmkid_caching_entry *pmkid_cache_ptr,int mfp);
...
int wpa_suite_to_cipher(u32 suite);
...
void _wpa_hexdump(int level, const char *title, const u8 *buf,size_t len, int show);
...
#define wpa_printf(Level, Fmt, ...)\
    do {\
        if(Level >= wpa_debug_level) {\
            printf("[%u] ",(unsigned int)asr_rtos_get_time());\
            printf(Fmt "\r\n", ## __VA_ARGS__);\
        }\
    }while(0)
