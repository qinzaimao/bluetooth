...
#define V(a,b,c,d) 0x##a##b##c##d
static const uint32_t KT0[256] = { KT };
#undef V

#define V(a,b,c,d) 0x##d##a##b##c
static const uint32_t KT1[256] = { KT };
#undef V

#define V(a,b,c,d) 0x##c##d##a##b
static const uint32_t KT2[256] = { KT };
#undef V

#define V(a,b,c,d) 0x##b##c##d##a
static const uint32_t KT3[256] = { KT };
#undef V

#undef KT

/* AES key scheduling routine */
int32_t aes_set_key( aes_context *ctx, uint8_t *key, int32_t nbits )
{
    int32_t i;
    uint32_t *RK, *SK;
...
void asr_aes_encrypt( aes_context *ctx, uint8_t input[16], uint8_t output[16] )
{
...
void asr_aes_decrypt( aes_context *ctx, uint8_t input[16], uint8_t output[16] )
{
    uint32_t *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
...
static uint8_t AES_enc_test[3][16] =
{
...
static uint8_t AES_dec_test[3][16] =
{
    ...
    for( m = 0; m < 2; m++ )
    {
        ...
        for( n = 0; n < 3; n++ )
        {
            ...
            for( i = 0; i < 400; i++ )
            {
                ...
                for( j = 0; j < 9999; j++ )
                {
                    if( m == 0 ) asr_aes_encrypt( &ctx, buf, buf );
                    if( m == 1 ) asr_aes_decrypt( &ctx, buf, buf );
                }
