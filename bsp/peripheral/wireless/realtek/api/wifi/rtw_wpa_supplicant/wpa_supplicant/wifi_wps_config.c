...
static struct task_struct ap_wps_task = {0};
...
int wps_start(u16 wps_config, char *pin, u8 channel, char *ssid)
{
	...
	for(int i=0; i< wps_max_cred_count; i++){
		os_xqueue_receive(queue_for_credential, &dev_cred[i], 60);
...
void wifi_start_ap_wps_thread(u16 config_methods, char *pin)
{
	...
	if(ap_wps_task.task != NULL){ //push item to wait queue to finish last ap_wps task 
		printf("\n\rWPS-AP: Wait for last ap_wps task exiting...\n");
		if(queue_for_credential)
			os_xqueue_send(queue_for_credential, &zero_mac, 0);
		while(ap_wps_task.task != NULL)
			//vTaskDelay(1);
			rt_thread_delay(1);
		//vTaskDelay(20);
		rt_thread_delay(20);
	...
	//if(xTaskCreate(wifi_start_ap_wps_thread_hdl, ((const char*)"ap_wps"), 512, NULL, tskIDLE_PRIORITY + 3, &ap_wps_task) != pdPASS)
	if(!rtw_create_task(&ap_wps_task, ((const char*)"ap_wps"), 512, /*tskIDLE_PRIORITY*/ + 3, wifi_start_ap_wps_thread_hdl, NULL))
