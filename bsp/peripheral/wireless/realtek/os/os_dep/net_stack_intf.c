...
//----- ------------------------------------------------------------------
#if (CONFIG_LWIP_LAYER == 1)
extern struct netif xnetif[NET_IF_NUM];			//LWIP netif
#endif
...
int rltk_wlan_send(int idx, struct eth_drv_sg *sg_list, int sg_len, int total_len)
{
	...
	if(idx == -1){
		pr_info("netif is DOWN");
		return -1;
	}
	pr_debug("%s is called", __FUNCTION__);
	...
	if (skb == NULL) {
		pr_info("rltk_wlan_alloc_skb()failed!\r\n");
		goto exit;
	}
	skb->data = data + 80;
	skb->head = data;
	...
	if (skb == NULL) {
		pr_info("rltk_wlan_alloc_skb() for data len=%d failed!", total_len);
		ret = -1;
		goto exit;
	}
